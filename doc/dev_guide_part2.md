...
このガイドはWSL2（Windows上のLinux環境）を使用していることを前提にしています。
すべてのコマンドはWSL2のターミナルで実行します。
...
> **補足**: インストールスクリプトは自動的に `~/.zshrc` または `~/.bashrc` にfnmの設定を追記します。
...
pwd
# → /home/jam00/work/absence-reporter と表示されればOK
...
```bash
# app/ ディレクトリにいることを確認
cd /home/jam00/work/absence-reporter/app
